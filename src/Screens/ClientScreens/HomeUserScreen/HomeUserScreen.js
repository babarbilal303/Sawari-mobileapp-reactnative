...
export default function HomeUserScreen() {
    ...
    const requestUserPermission = async () => {
        ...
        if (enabled) {
            ...
            //for background notification it was trigerred when app was in background 
            messaging().onNotificationOpenedApp(onPressNotification)

            //for closed app notification it was trigreed when app was in quit mode

            messaging().getInitialNotification().then(notif => {
                alert("app quit mode", JSON.stringify(notif))

                console.log("WORKING CLOSED APP")
                if (notif) {
                    onPressNotification(notif)
                }
            })
            // if app in fore ground state
